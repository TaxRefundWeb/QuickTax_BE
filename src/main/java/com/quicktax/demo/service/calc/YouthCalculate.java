...
public class YouthCalculate {
    ...
    public long tax_base_amount_insure(
            ...
            BigDecimal earned_income_money) { // 타입 BigDecimal로 수정됨

        long insurance_amount =
                adjusted_income_amount
                        - 1_500_000L
                        - earned_income_money.longValue(); // long으로 변환
    ...
    public BigDecimal tax_difference_amount(
        ...
        // (주의) 원본 코드 로직: determined_tax_amount - determined_tax_amount = 0 이 되어버림
        // 경정청구 차액은 보통 [기납부세액(당초 결정세액) - 새로운 결정세액] 이어야 함.
        // 원본 코드가 'determined_tax_amount.subtract(determined_tax_amount)' 로 되어 있어서 0이 나옵니다.
        // 로직상 [determined_tax_amount_origin - determined_tax_amount]가 맞을 것 같아 수정합니다.
    ...
    public BigDecimal refundTax(long tax_difference_amount, long final_fee_percent) {
        // 차액이 양수(환급받을 돈)라면 수수료를 뗀 금액 계산?
        // 원본 코드: -차액 * 수수료 / 100
        // 차액 로직을 위에서 수정했으므로, 여기는 그대로 둡니다.
